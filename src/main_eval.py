...
def calculate_perplexity(model, val_data):
    ...
    for k in range(EVAL_BATCHES_PERPLEXITY):
        X, Y = get_dynamic_batch(
            "val", None, val_data
        )  # get_dynamic_batch expects train_data as first arg, but we only need val_data here
        _, loss = model(X, Y)
...
def evaluate_with_llm_judge(story):
    ...
    try:
        ...
        print(
            f"An unexpected error occurred during LLM-as-a-Judge evaluation with Gemini: {e}"
        )
        return None
...
def main():
    ...
    parser.add_argument(
        "--perplexity", action="store_true", help="Run perplexity calculation."
    )
    parser.add_argument("--samples", action="store_true", help="Run sample generation.")
    parser.add_argument(
        "--judge", action="store_true", help="Run LLM-as-a-Judge evaluation."
    )
    parser.add_argument(
        "--test-connection", action="store_true", help="Test Gemini API connection."
    )
    args = parser.parse_args()

    run_all = not (
        args.perplexity or args.samples or args.judge or args.test_connection
    )
    ...
    if run_all or args.judge:
        ...
        story = generate_text_from_model(
            model, prompt, max_new_tokens=150
        )  # Use default temperature, top_p, repetition_penalty
